...
random_samples <- tibble(replicate = 1:5,
       samp = map(replicate, ~ sample_n(urn, size = 30)))
```
...
scene2 <- function(size = 50){
  sample_n(urn, size = size, replace = TRUE) %>%
...
prob_red_wins <- tibble(replicate = 1:1000,
                        samp = map(replicate, ~ scene2(size = 50))) %>%
  unnest(samp) %>%
